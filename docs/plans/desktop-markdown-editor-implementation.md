...
**Approach chosen:** `mermaid-rs-renderer 0.2.0` (pure Rust, no Node.js) → SVG string → `resvg 0.46` rasterized PNG

**Why not GPUI `svg()` element:** GPUI's `svg()` converts the SVG to a **monochrome alpha mask** tinted to `text_color`. Multi-colour Mermaid diagrams (fills, borders, text) are invisible this way — `svg()` is icon-only. The only correct path is `img(PathBuf)` with a rasterized PNG.

**Why not `write_output_png` from mermaid-rs-renderer:** The library's own PNG helper re-parses the SVG through `usvg::Tree::from_str`. The rendered SVG contains unescaped double-quotes inside `font-family` attributes (e.g. `font-family="..., "Segoe UI", ..."`), which breaks the XML parser at character 1408. Fixed by a `sanitize_svg_xml()` pre-pass that replaces inner unescaped `"` with `'` (CSS-valid for font-family).

**Rendering pipeline:**
```
mermaid source
  → mermaid_rs_renderer::render()       # pure-Rust layout engine → SVG string
  → sanitize_svg_xml()                  # fix bad XML from font-family attrs
  → usvg::Tree::from_str()             # parse into resvg scene tree
  → scale = min(900/natural_w, 2.0)    # HiDPI: 2× unless diagram > 450px wide
  → resvg::render() into Pixmap        # rasterize at 2× physical pixels
  → pixmap.save_png()                  # write to /tmp/octodocs-mermaid-cache/{hash}.png
  → {hash}.dims sidecar                # stores "natural_w natural_h" for layout
  → img(Arc<Path>).w(px(natural_w))    # display at logical (1×) size in preview
```

**Cache:** `/tmp/octodocs-mermaid-cache/{hash_v5}.png` + `.dims` sidecar (key = `"mermaid-cache-v5" + source`). Cache hit reads only `.dims` — no re-render.

**Result:** ~198×269 logical px display, crisp 395×538px PNG on disk. All 10 `octodocs-core` tests pass.
...
**Approach (Phase 10+): Single-pane WYSIWYG Block Editor (Typora/Notion-style)**

The split-pane approach shipped for v1 but the product direction is now **full WYSIWYG** — markdown syntax fully hidden. The user sees rendered output at all times; clicking a block activates an inline raw-text editor scoped to that block only.  All other blocks remain rendered. This is the Typora model.

**Why now feasible:** `pulldown-cmark`'s offset iterator gives byte-exact source ranges per block, so the document can be split into `Vec<DocumentBlock { source, node, range }>`. A single shared `EditorState` is reused for whichever block is active; on deactivation, the edited source is spliced back into `document.content` and the tree is re-parsed. No per-block entity allocation needed.

**Previous rationale for split-pane (still valid for reference):** True WYSIWYG requires mapping cursor positions between raw source and styled DOM — architecturally complex. The split-pane model delivered the same live-preview UX with significantly less complexity.
...
- [x] Phase 5: [Mermaid Diagram Rendering](#phase-5-mermaid-diagram-rendering)
...
- [x] Phase 10: [WYSIWYG Block Editor](#phase-10-wysiwyg-block-editor)
...
### Option 2: Single-Pane WYSIWYG Block Editor (Typora/Notion-style) ← **Current target**

**Description:** Single full-width pane. Each top-level Markdown block is rendered as formatted output. Clicking a block activates an inline `Editor` for that block's raw source only. Other blocks remain rendered. Deactivating saves the edited block back into the document and re-parses.

**Implementation model:**
- `DocumentBlock { source: String, node: RenderNode, byte_range: Range<usize> }` — parsed by `Renderer::parse_blocks()`
- `AppState.active_block: Option<usize>` + `AppState.block_editor: Entity<EditorState>` (shared singleton)
- On click: load block source into shared editor, set `active_block`
- `block_editor.on_change` → update `blocks[active_block].source`, reassemble full doc, re-parse
- On blur/Escape → `active_block = None`
- Toolbar actions apply to `active_block`'s source

**Pros:**
- Markdown syntax fully hidden — pure formatted view
- Single editor entity reused — no per-block allocation
- Toolbar and Mermaid rendering unchanged

**Cons:**
- Block boundaries can feel jarring for multi-block selections (v1 tradeoff)
- Cursor position within a block editor doesn't directly reflect position in the full document (handled by splicing on save)
...
**Chosen Approach (v1 shipped):** Option 1 — Split-Pane  
**Chosen Approach (v2 / Phase 10):** Option 2 — WYSIWYG Block Editor

Option 1 shipped successfully. Option 2 is now in active implementation.
...
### Phase 5: Mermaid Diagram Rendering ✅ COMPLETE

- [x] Add `mermaid-rs-renderer 0.2.0` + `resvg 0.46` + `usvg 0.46` to `octodocs-core`
- [x] `octodocs_core::mermaid::render_png(source, output) -> anyhow::Result<(f32, f32)>` — renders to HiDPI PNG, returns logical `(width, height)`
- [x] `sanitize_svg_xml()` — fixes unescaped double-quotes in font-family attributes before passing SVG to usvg
- [x] `ensure_mermaid_png_path()` in `preview_pane.rs` — hash-keyed cache (`/tmp/octodocs-mermaid-cache/`) with `.dims` sidecar for logical size
- [x] `img(Arc<Path>).w(px(logical_w))` in preview pane — displays at natural size, crisp at 2× pixel density
- [x] Graceful fallback: render errors fall through to `render_mermaid_fallback()` (raw source + error badge)
- [x] 10/10 unit tests passing in `octodocs-core`

**Key technical decisions:**
- GPUI `svg()` discarded — it is monochrome alpha-mask only, unsuitable for multi-colour diagrams
- `write_output_png` from the library discarded — re-parses the same SVG and hits the XML bug
- Direct `resvg::render()` used instead, giving full control over scale and pixmap size
...
**Goal:** Replace the split-pane layout with a single full-width WYSIWYG pane where markdown syntax is completely hidden. The user sees rendered output at all times; clicking any block activates an inline raw-text editor scoped to that block. All other blocks remain rendered. This is the Typora/Notion model.

**Status: ✅ COMPLETE** — shipped and building clean as of 2026-02-25.

**Data model additions to `octodocs-core`:**
- [x] `DocumentBlock { source: String, node: RenderNode }` in `renderer.rs`
- [x] `Renderer::parse_blocks(text: &str) -> Vec<DocumentBlock>` — uses `pulldown-cmark`'s `into_offset_iter()` for byte-exact block boundaries
- [x] `DocumentBlock::reassemble(blocks: &[DocumentBlock]) -> String` — joins all block sources to reconstruct full document
- [x] Exported from `octodocs-core::lib.rs`: `DocumentBlock, Inline, RenderNode, RenderTree, Renderer`

**`AppState` additions (`app_state.rs`, 130 lines):**
- [x] `blocks: Vec<DocumentBlock>` — replaces direct `render_tree: RenderTree`
- [x] `active_block: Option<usize>` — index of the block currently being edited (`None` = all rendered)
- [x] `editor_state: Entity<EditorState>` — shared singleton editor, reused for whichever block is active
- [x] Subscription: editor changes → writes back to `blocks[active].source`, calls `reassemble()`, syncs `document.content`, sets `dirty = true`
- [x] `activate_block(idx, cx)` — loads `blocks[idx].source`, calls `place_cursor_at_end()`
- [x] `deactivate_block(cx)` — sets `active_block = None`, calls `cx.notify()`

**New view `block_editor_pane.rs` (177 lines):**
- [x] Full-width scrollable column; centered at max 760 px, 48 px side padding
- [x] For each block at index `i`:
  - If `i == active_block`: `border_l_4()` with `primary` color + `card` bg + `Editor` sized to block line count
  - Else: `border_l_4()` with transparent border (holds layout space) + hover tint/bar via `.hover(|s| s.border_color(border).bg(muted))`
- [x] Consistent padding in both states — **no layout jump** on activation
- [x] Adaptive editor height: `min_lines = block.source.lines().count().max(1)` (no hard-coded 3-line box)
- [x] Always-visible trailing click zone (44 px) — appends new block or re-activates trailing empty block
- [x] Cursor placed at end of block content on activation (`place_cursor_at_end()` patched into adabraka-ui `EditorState`)

**`root.rs` changes (212 lines):**
- [x] `h_resizable` split pane removed
- [x] `EditorPane` + `PreviewPane` replaced with `BlockEditorPane` as central content area
- [x] Toolbar actions target shared `editor_state` (active block's editor)
- [x] StatusBar retained at the bottom

**adabraka-ui patch (`editor.rs`):**
- [x] Added `pub fn place_cursor_at_end(&mut self, cx: &mut Context<Self>)` — called in `activate_block` so cursor always opens at the end of the block text, not position 0

**Technical decisions:**
- Left-bar accent (4 px) used instead of a box border — matches blockquote style in the renderer, avoids heavy blue rectangle
- `border_l_4()` with `opacity(0.0)` at rest reserves the bar width so padding is geometrically identical between active and inactive states
- Single `EditorState` singleton reused across all blocks — no per-block entity allocation
- `render_node()` in `preview_pane.rs` made `pub` and reused by `block_editor_pane.rs` for inactive block rendering

**Remaining v1 limitations (deferred):**
- Keyboard: `Escape` to deactivate and `Enter`-at-end-of-block to create new block not yet wired
- Block splitting (Enter mid-block) and merging (Backspace at block start) not implemented
- Undo history is per-block only (no cross-block undo)
- Mermaid blocks are click-to-edit raw text (no special fence guard on merge)
...
- [x] App window opens with Toolbar, content pane, and StatusBar visible
- [x] Typing Markdown updates the preview in real time
- [x] A Mermaid flowchart fence renders as a full-colour PNG diagram (crisp at 2× density)
- [x] New / Open / Save / Save As all function correctly via toolbar buttons
- [x] Light/dark theme toggle works
- [x] All `octodocs-core` unit tests pass (10/10)
- [x] **WYSIWYG block editor: clicking a rendered block activates an inline editor (Phase 10)**
- [x] **Markdown syntax is fully hidden in document view (Phase 10)**
...
- File operations work via toolbar buttons (MenuBar removed due to adabraka-ui limitations).
- Mermaid: renders as full-colour PNG via `resvg` rasterization — complete.
- WYSIWYG mode: in active implementation (Phase 10).
